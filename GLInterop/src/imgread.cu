...
static cudaArray *image_array;
static texture<uchar4, cudaTextureType2D, cudaReadModeElementType> image_texture;
...
__device__ void extractTile4x4(int x, int y, int img_width, int img_height, uint8_t out_tile[64]);
__device__ void extractTile16x16(uint32_t offset, const uint8_t *pixels, int width, uint8_t out_tile[1024]);
...
__device__ uint32_t colorFromRgba(const uint8_t *rgba, int width, int height, int x, int y);
...
struct GrayscaleFunctor
{
    ...
    int height;
    size_t size;
    GrayscaleFunctor(int width_in, int height_in, thrust::device_vector<uint8_t>& gray_out)
    {
...
struct Dxt1Functor
{
    ...
    int dxt1_height;
    size_t size;
    Dxt1Functor(int width_in, int height_in, thrust::device_vector<uint8_t>& dxt1_out)
    {
    ...
    __device__ void operator()(int thread_id)
    {
        ...
        if (thread_id < size)
        {
            ...
            extractTile4x4(px_x, px_y, img_width, img_height, tile);
...
void initImageConverter(int width, int height, GLuint texture)
{
    ...
    cudaGraphicsSubResourceGetMappedArray(&image_array, resource, 0, 0);
    cudaGraphicsUnmapResources(1, &resource, 0);
    cudaBindTextureToArray(image_texture, image_array);
...
void rgbaToGrayscale(uint8_t *rgba, uint8_t *gray)
{
    ...
    thrust::for_each_n(thrust::device, it, img_w * img_h, GrayscaleFunctor(img_w, img_h, *image_output_ptr));
...
void rgbaToDxt1(uint8_t *rgba, uint8_t *dxt1)
{
    ...
    thrust::for_each_n(thrust::device, it, n, Dxt1Functor(img_w, img_h, *image_output_ptr));
    cudaDeviceSynchronize();
